...
public class EnterWALossCheckc implements WallAdvantageLossChecki {
    ScenarioC scen ;
    ...
    public EnterWALossCheckc(){
        scen = ScenarioC.getInstance();
    }

    public boolean WallAdvantageLossCheck(Hex Oldhex) {
        // determines if broken/unarmed friendlies lose Wall Adv and
        // if Concealed WA switches (meaning friendly ? is a dummy)

        LinkedList<PersUniti> unitsinhex = infcommfunc.FindUnitsInHex(Oldhex);
        for (PersUniti WAUnit: unitsinhex) {
            if ((WAUnit.getbaseunit().getOrderStatus() == Constantvalues.OrderStatus.GoodOrder || WAUnit.getbaseunit().getOrderStatus() == Constantvalues.OrderStatus.Berserk) &&
                ...
                if (!EnemyForcesWALoss(Oldhex)) {return false;} //concealed units prevents WA loss
            }
        }
    ...
    private boolean EnemyForcesWALoss(Hex Oldhex) {
        ...
        Hex [] Adjacenthexes = mapcomfunc.getAdjacentHexArray(Oldhex);
        for (int x = 0; x < 6; x++) {
            // identify the other hex
            Hex hextotest = Adjacenthexes[x];
            IsSide Sidechk = new IsSide();
            if (Sidechk.IsWAAllowedToSwitch(Oldhex, x, hextotest)) {
    ...
    public boolean CanClaimWallAdv(Hex hextocheck, Constantvalues.Nationality NatTest) {
        ...
        /*
        ElseIf TypeCheck.IsThingATypeOf(Constantvalues.Typetype.Concealment, Defendersprite.TypeID) Then
                CanClaim = True
        ...
        BasePersUnit.TypeType_ID = Constantvalues.Typetype.Concealment).First
        ...
        If DefenderCheck.basepersunit.hexlocation = Constantvalues.Feature.Foxhole Or EnemyUnit.
        basepersunit.hexposition = Constantvalues.AltPos.AboveWire Or
                (EnemyUnit.basepersunit.hexlocation = Constantvalues.Location.Pill1571 And EnemyUnit.basepersunit.hexlocation <= Constantvalues.Location.Bombprf)
        Or
        EnemyUnit.basepersunit.hexlocation = Constantvalues.Location.BunkUnder Or
        EnemyUnit.basepersunit.hexposition = Constantvalues.AltPos.InFoxhole Or EnemyUnit.
        basepersunit.hexposition = Constantvalues.AltPos.AbovePanji Or
        EnemyUnit.basepersunit.hexlocation = Constantvalues.Feature.Trench Or EnemyUnit.
        basepersunit.hexposition = Constantvalues.AltPos.InTrench Then CanClaim = False :Continue For
        If(EnemyUnit.basepersunit.hexlocation >= Constantvalues.Location.StBr14 And EnemyUnit.basepersunit.hexlocation <= Constantvalues.Location.FoBr)
        Then
        For i = 1 To 6
        Dim Sidetest As Integer = Game.Linqdata.Gethexsidetype(i, CInt(EnemyUnit.basepersunit.hexnum))
        If Not (Sidetest >= Constantvalues.Hexside.Roadblock0 And Sidetest <=
        Constantvalues.Hexside.Roadblock6)Then 'no roadblock
        CanClaim = False :Continue For 'unit is on bridge with no roadblock side
        End If
        Next
        End If
        'check if creststatus
        If DefenderCheck.basepersunit.hexposition >= Constantvalues.AltPos.CrestStatus0 And EnemyUnit.
        basepersunit.hexposition <= Constantvalues.AltPos.CrestStatus5 Then
        'now need to check that Wall/Hedge exists within crest hexside - not other side of depression
        Dim SideConvert As New UtilClassLibrary.ASLXNA.ConversionC
        Dim crestside As Integer = SideConvert.CrestSideToSide(CInt(EnemyUnit.basepersunit.hexposition))
        ...
        If SideTest.IsAWallHedgeRdBlk(testside, CInt(EnemyUnit.BasePersUnit.LOCIndex)) Then ConList.
        Add(EnemyUnit) :Cando = True :Exit Do
        x += 1
        Loop Until x = 2
        If CanClaim = False Or Cando = False Then CanClaim = False
        ElseIf IsTerrChk.
        IsLocationTerrainA(CInt(EnemyUnit.basepersunit.hexnum), CInt(EnemyUnit.basepersunit.hexlocation), Constantvalues.Location.Creststatustype)
        Then
        'WA requires creststatus and defender is not in creststatus
        ...
        IsThingATypeOf(Constantvalues.Typetype.Personnel, Checksprite.TypeID) Then
        ...
        (UnitCheck.BasePersUnit.OrderStatus = Constantvalues.OrderStatus.GoodOrder Or UnitCheck.
        BasePersUnit.OrderStatus = Constantvalues.OrderStatus.Berserk)Then
        Return False
        ...
        For Each Checksprite As ObjectClassLibrary.ASLXNA.SpriteOrder In OH.VisibleCountersInHex
        If(Checksprite.hexloc = hexloc Or IsPositionInBaseLocation(Checksprite.hexloc)) And TypeCheck.
        IsThingATypeOf(Constantvalues.Typetype.Concealment, Checksprite.TypeID) Then
    ...
    private boolean DoesConcealmentBlock(LinkedList<PersUniti> Conlist, Hex hextouse) {
        // Sorts out role of Con counters in preventing WA claim

        // PART OF DEALING WITH CONCEALMENT COUNTERS
    ...
    If TypeCheck.IsThingATypeOf(Constantvalues.Typetype.Personnel, Defendersprite.TypeID) Then
    ...
    Dim DefenderName As String = InputBox("Enter name of unit temporarily revealed in " & Defenderhexname & ": ", "Testing if Concealed Stack can claim WA")
                    'verify
    For Each Defendersprite As ObjectClassLibrary.ASLXNA.SpriteOrder In OH.VisibleCountersInHex
    If TypeCheck.IsThingATypeOf(Constantvalues.Typetype.Personnel, Defendersprite.TypeID) Then
    ...
    End If
    ElseIf TypeCheck.IsThingATypeOf(Constantvalues.Typetype.Concealment, Defendersprite.TypeID) Then
                            'Dim ConGet As IQueryable(Of DataClassLibrary.OrderofBattle) = Game.Linqdata.RetrieveConcealedUnits(CInt(Defendersprite.ObjID))
    Dim ConGet As IQueryable(Of ObjectClassLibrary.ASLXNA.PersUniti) = CType((From selunit As ObjectClassLibrary.ASLXNA.PersUniti In scencolls.Unitcol Where selunit.BasePersUnit.Con_ID = Defendersprite.ObjID Select selunit), Global.System.Linq.IQueryable(Of Global.ObjectClassLibrary.ASLXNA.PersUniti))
    Try
    Dim MatchUnit As ObjectClassLibrary.ASLXNA.PersUniti = (ConGet).Where(Function(DoMatch) Trim(DoMatch.BasePersUnit.UnitName) = Trim(DefenderName)).First
                                MessageBox.Show("Verified. Proceed")
    ClaimReal = True : Exit Do
    ...
    Dim AllDummy As Boolean = True : Dim MandatoryTest As Boolean : Dim Firstime As Boolean = True : Dim MixedMan As Boolean = False : Dim MustReveal As Boolean = False
    ...
    If SideChk.IsWAMandatory(CInt(ConItem.basepersunit.hexnum), Otherhexnum, CInt(ConItem.basepersunit.hexlocation), CInt(ConItem.basepersunit.hexposition)) Then
    If Firstime Then
    ...
    Else
    For Each ConItem As ObjectClassLibrary.ASLXNA.PersUniti In Conlist
    Dim ConGet As IQueryable(Of ObjectClassLibrary.ASLXNA.PersUniti) = CType((From selunit As ObjectClassLibrary.ASLXNA.PersUniti In scencolls.Unitcol Where selunit.BasePersUnit.Con_ID = ConItem.BasePersUnit.Unit_ID Select selunit), Global.System.Linq.IQueryable(Of Global.ObjectClassLibrary.ASLXNA.PersUniti))
    Try
    Dim MatchUnit As ObjectClassLibrary.ASLXNA.PersUniti = (ConGet).Where(Function(DoMatch) Trim(DoMatch.BasePersUnit.UnitName) = Trim(UnitName)).First
                                MessageBox.Show("Verified. Proceed")
