...
public class SearchHexesc implements MoveWithinHexi {
    ...
    public SearchHexesc(Hex hexclicked, Constantvalues.UMove Movementoptionclicked) {
    ...
    public boolean MoveAllOK() {
        ...
        MFCost = 1; //Moveloc.getlocationentrycost(hexclickedvalue);
        //MessageBox.Show("Trying to Search from . . . " + hexclickedvalue.getName() + " . . . which will cost " + Double.toString(MFCost) + " MF");
        ...
        LegalMovei LegalCheck = new SearchLegalc(hexclickedvalue, movementoptionclickedvalue);
    ...
    public void MoveUpdate() {
        ...
        for (PersUniti MovingUnit:Scencolls.SelMoveUnits) {
            MovingUnit.getMovingunit().setMFAvailable(0);
            MovingUnit.getMovingunit().setMFUsed(MovingUnit.getMovingunit().getMFAvailable() + MFCost);
            Temploc = MovingUnit.getbaseunit().gethexlocation();
