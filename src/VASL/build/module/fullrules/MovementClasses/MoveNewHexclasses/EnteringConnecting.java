...
public class EnteringConnecting implements MoveNewHexi {
    ...
    public boolean MoveAllOK() {
        ...
        if (LegalCheckunit.getbaseunit().getLevelinHex() >= 1) {  // ConstantClassLibrary.ASLXNA.Location.Roof to ConstantClassLibrary.ASLXNA.Location.Building3rdLevel Then
            ...
            if (!upperlevelcheck.IsMovementLegal()) {
                ...
                // no position change - ??
                LocationChange = upperlevelcheck.getLocationchangevalue();
            }
            ...
            if (!lowerlevelcheck.IsMovementLegal()) {
                ...
                // no position change - ??
                LocationChange = lowerlevelcheck.getLocationchangevalue();
            }
            ...
            if (MoveNewCheck.getLocationchangevalue() != Moveloc.getvasllocation()) {
                // MessageBox.Show("new location due to trench/tunnel movement")
                Moveloc = new Locationc(MoveNewCheck.getLocationchangevalue(), movementoptionclickedvalue);
            }
    ...
    public void MoveUpdate() {
        ...
        for (PersUniti MovingUnit: Scencolls.SelMoveUnits) {
            ...
            Constantvalues.MovementStatus MoveStatus = Constantvalues.MovementStatus.Connecting;
